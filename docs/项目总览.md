...
- 资金闭环的生产运营细节：部分退款、自动差异处理、老师端确认收款 UI
- 真实实名供应商全流程落地
- 生产级短信限流 / 风控 / 审计
...
微信支付主链路现在在 `PaymentsService` + `WechatPayClient`：

- `POST /payments/bookings/:bookingId/prepare`：创建或复用 `PaymentIntent`，调用微信 JSAPI/小程序下单，返回 `wx.requestPayment` 参数。
- `POST /payments/wechat/notify`：验签、解密、校验 AppID/商户号/金额/币种，落 `PaymentProviderEvent`，再投影订单状态。
- `POST /payments/bookings/:bookingId/reconcile`：主动按商户订单号查单并回写订单状态。
- 支付成功且预约转 `CONFIRMED` 时，会 `upsert` 一条 `Lesson`。

注意：

- `updatePayment()` 只保留给后台/测试内部调用，不再作为 controller 路由暴露给小程序端。
- 已补齐未结算全额退款、微信退款回调/查单、老师钱包入账、提现申请/后台审核/微信商家转账、交易/退款账单对账；部分退款和自动化差异处理仍是后续项。
...
当前已有微信小程序支付 provider 主链路：

- 普通商户模式 JSAPI/小程序下单
- 微信支付回调验签解密与幂等事件日志
- 主动查单补偿
- 支付成功后投影 Booking 并自动生成 Lesson
- `test-support` 仍可在开发态模拟支付结果
- `Wallet/Payout` 已接入未结算全额退款、老师钱包入账、提现审核与微信商家转账；`WECHAT_PAY_TRANSFER_ENABLED` 默认关闭，生产打开前要确认商家转账权限和场景报备
...
5. 资金域已具备基础闭环，但提现真实出款由 `WECHAT_PAY_TRANSFER_ENABLED` 控制，生产打开前必须核对商家转账权限、场景 ID、报备信息和回调地址。
