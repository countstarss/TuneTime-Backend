...
| 指标              | 当前值 | 说明                                                     |
| ----------------- | ------ | -------------------------------------------------------- |
| Controller 入口组 | 18     | `root + 17` 个业务 controller 入口组                     |
| HTTP 接口数       | 174    | 按当前 controller 路由统计                               |
| Prisma model 数   | 42     | 含已开放域与资金/结算模型                                |
...
| `payments`             |          16 | `GUARDIAN` `TEACHER` `ADMIN` `SUPER_ADMIN` + 微信回调 | `PaymentIntent` `PaymentRefund` `PaymentProviderEvent` `Wallet` `WalletTransaction` `TeacherPayoutAccount` `Payout` `ReconciliationRun` | 微信小程序支付、未结算全额退款、老师钱包、提现商家转账、微信账单对账 | 普通商户模式；回调/查单会校验商户身份、订单号、币种和金额；提现默认需显式启用                  | `services/payment.ts`                                 |
...
| POST   | `/payments/wechat/refund-notify`                | `PUBLIC`              | 接收微信退款结果通知           |
| POST   | `/payments/wechat/transfer-notify`              | `PUBLIC`              | 接收微信商家转账结果通知       |
| GET    | `/wallet/me`                                    | `TEACHER`             | 查看老师钱包余额               |
| GET    | `/wallet/me/transactions`                       | `TEACHER`             | 查看老师钱包流水               |
| POST   | `/payouts`                                      | `TEACHER`             | 创建提现申请                   |
| GET    | `/payouts/me`                                   | `TEACHER`             | 查看我的提现申请               |
| POST   | `/admin/payments/bookings/:bookingId/refunds`   | `ADMIN` `SUPER_ADMIN` | 为预约发起未结算全额退款       |
| POST   | `/admin/payments/refunds/:id/reconcile`         | `ADMIN` `SUPER_ADMIN` | 主动同步退款单                 |
| POST   | `/admin/settlements/bookings/:bookingId/settle` | `ADMIN` `SUPER_ADMIN` | 手动触发预约结算入账           |
| POST   | `/admin/payouts/:id/approve`                    | `ADMIN` `SUPER_ADMIN` | 审核通过提现并发起微信商家转账 |
| POST   | `/admin/payouts/:id/reject`                     | `ADMIN` `SUPER_ADMIN` | 拒绝提现申请                   |
| POST   | `/admin/payouts/:id/reconcile`                  | `ADMIN` `SUPER_ADMIN` | 主动同步微信商家转账           |
| POST   | `/admin/reconciliation/wechat/runs`             | `ADMIN` `SUPER_ADMIN` | 触发微信交易/退款账单对账      |
